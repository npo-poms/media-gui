angular.module( 'poms.media.directives' )
    .directive( 'pomsTargetgroups', ['PomsEvents', 'EditService', 'FavoritesService' ,'MediaService', 'ListService', '$q', '$sce', '$timeout', '$modal', 'EditFieldService', function ( pomsEvents, editService, favoritesService, mediaService, listService, $q, $sce, $timeout, $modal, editFieldService ) {
        return {
            restrict: 'E',
            templateUrl: 'edit/editables/poms-targetgroups.html',
            scope: {
                media: '=',
                helpField : '@',
                header: '@'
            ...
            controller: function ( $scope ) {
                ...
                function load ( media, dest ) {
                    mediaService.getTargetGroups( media ).then(
                        function ( data ) {
                ...
                load( media, $scope.targetGroups);

                listService.getTargetGroups().then(
                    function ( data ) {
                        $scope.options = data;
                    },
                    function ( error ) {
                ...
                $scope.submit = function ( e ) {
                    ...
                    if ( angular.equals( data, $scope.targetGroups ) ) {
                        $scope.waiting = false;
                        $scope.isOpen = false;
                        return; // no change
                    }

                    editService.targetGroups( media, data ).then(
                        function ( result ) {
