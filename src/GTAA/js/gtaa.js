var gtaa = {
    ...
    open: function ( callback, options ) {
        ...
        var popup;
        var iframe;
        var domain = '{domain}' || window.gtaaDomain || '';
        var query = '';
        ...
        if ( document.all ) {
            ...
            iframe.src = domain + '/GTAA/IE.html' + query;
        } else {
            popup = window.open( domain + '/GTAA/' + query, '', 'width=1024,height=800,titlebar=no,toolbar=no,statusbar=no,directories=no,location=no' );
        }
